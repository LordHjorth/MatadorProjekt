...
import cards.CardMoveAbsolute;
import cards.CardMoveRelative;
...
import cards.PayTax;
import cards.ReceiveMoneyFromPlayer;
...
public class MiniMonopoly {
	...
	public static Game createGame() {
		...
		//TODO kig på dette kort og dens funktion
		PardonCard pardonOne = new PardonCard();
		pardonOne.setText("I anledning af kongens fødselsdag benådes De herved for fængsel. Dette kort kan opbevares, indtil De får brug for det, eller De kan sælge det. ");
		cards.add(pardonOne);
		...
		//TODO hvordan rykker man tre felter tilbage		
		CardMoveRelative moveBack = new CardMoveRelative();
		moveBack.setTarget(game.getCurrentPlayer().getCurrentPosition());
		moveBack.setText("Ryk tre felter tilbage.");
		cards.add(moveBack);
		
	//TODO kig på dette kort og dens funktion
		PardonCard pardonTwo = new PardonCard();
		pardonTwo.setText("I anledning af kongens fødselsdag benådes De herved for fængsel. Dette kort kan opbevares, indtil De får brug for det, eller De kan sælge det. ");
		cards.add(pardonTwo);
		...
		CardMoveAbsolute prison = new CardMoveAbsolute();
		prison.setTarget(game.getSpaces().get(10));
		prison.setText("Gå i fængsel. Ryk direkte til fængslet. Selv om De passerer Start, indkasserer De ikke kr. 4.000.");
		cards.add(prison);

		CardMoveAbsolute Rådhuspladsen = new CardMoveAbsolute();
...
//		shipping.setText("Ryk brikken frem til det nærmeste redderi og betal ejeren to gange den leje, han ellers er berettiget til. Hvis selskabet ikke ejes af nogen kan De købe det af banken.");
//		cards.add(shipping);		

		CardMoveAbsolute Frederiksberg = new CardMoveAbsolute();
...
//		ReceiveMoneyFromPlayer birthday = new ReceiveMoneyFromPlayer();
//		birthday.setText("Det er deres fødselsdag. Modtag af hver medspiller kr. 200.");
//		birthday.setAmount(200);
//		cards.add(birthday);
//		game.setCardDeck(cards);
		
		CardMoveAbsolute start = new CardMoveAbsolute();
		...
		legat.setText("De modtager Matador-legatet for værdig trængende, stort kr. 40.000. Ved værdig trængende forstås, at Deres formue, d.v.s. Deres kontante penge + skøder + bygninger ikke overstiger kr. 15.000.");
		cards.add(legat);
...
//		oilTax.setText("Oliepriserne er steget, og De skal betale: kr. 500 pr. hus, kr. 2.000 pr. hotel.");
//		cards.add(oilTax);

		//TODO skal der oprettes en ny klasse??	
//		CardMove shippingOther = new CardMove();
//		shippingOther.setTarget(game.getSpaces().get(39));
//		shippingOther.setText("Tag med den nærmeste færge - Flyt brikken frem og hvis De passerer "Start" inkassér da kr. 4.000.");
...
//		cards.add(shipping);
		...
		CardMoveAbsolute grønningen = new CardMoveAbsolute();
		...
		CardMoveAbsolute prisonTwo = new CardMoveAbsolute();
		prisonTwo.setTarget(game.getSpaces().get(10));
		prisonTwo.setText("Gå i fængsel. Ryk direkte til fængslet. Selv om De passerer Start, indkasserer De ikke kr. 4.000.");
		cards.add(prisonTwo);
